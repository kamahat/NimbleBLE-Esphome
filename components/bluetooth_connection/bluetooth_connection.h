// bluetooth_connection.h -- SURCHARGE NimbleBLE-Esphome. Shared types and
// helpers for the GATT connection backend and the Bluetooth proxy that
// drives it. Identique au core sauf : pas de bras RP2 (hors périmètre de ce
// projet -- ESP32/NimBLE uniquement, voir docs/ARCHITECTURE.md), et
// unpair_device()/clear_gatt_cache() portés sur les API NimBLE plutôt que
// Bluedroid (host/ble_gap.h au lieu de esp_ble_gap_api.h).
...
// integers on the wire. Aliases esp_err_t on esp32 (matching the core's own
// spelling so the api/ layer needs no change), even though our NimBLE calls
// return plain int (NimBLE host error codes) rather than esp_err_t --
// nimble_gattc.cpp's callers already forward those as plain int, which
// converts losslessly into esp_err_t (both are typedef'd int on ESP-IDF).
#ifdef USE_ESP32
...
// What this build's connection backend supports beyond GATT operations; the
// proxy derives its feature flags from these. NimBLE supports pairing
// (ble_gap_security_initiate, already used by nimble_gattc.cpp) but has no
// GATT-cache-clearing equivalent ported yet (see docs/OVERRIDE_CAVEATS.md) --
// clear_gatt_cache() below is a documented no-op, not a silent gap.
